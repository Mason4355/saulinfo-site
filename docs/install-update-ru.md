...
Если nginx уже настроен отдельно, на вопрос `Create/update nginx config and certbot automatically` можно ответить `n`. Установщик не будет трогать nginx/certbot и в конце напечатает готовые `location` для ручного проксирования.
