...
def create_app() -> Flask:
    ...
    def personal_data_consent_version() -> str:
        ...
        try:
            ...
            for key in sorted(settings_snapshot):
                if key.startswith("site_privacy_") or key.startswith("site_consent_"):
                    parts.append(f"{key}={settings_snapshot.get(key) or ''}")
        except Exception:
            app.logger.exception("Failed to include editable legal settings in consent version")
    ...
    def build_site_content() -> dict:
        ...
        return {
            ...
            "privacy_title": site_setting("site_privacy_title", "Политика обработки персональных данных SaulInfo"),
            "privacy_intro": site_setting("site_privacy_intro", "Документ описывает, какие данные нужны сайту, панели и поддержке для работы сервиса. Политика подготовлена для работы с пользователями в РФ с учётом 152-ФЗ."),
            "privacy_general_text": site_setting("site_privacy_general_text", "SaulInfo обрабатывает персональные данные пользователей на территории Российской Федерации и в соответствии с применимыми требованиями российского законодательства о персональных данных. Данные нужны для предоставления доступа к личному кабинету, покупки и продления ключей, поддержки пользователей, обработки платежей и защиты сервиса от злоупотреблений."),
            "privacy_contact_note": site_setting("site_privacy_contact_note", "Контакт для запросов по персональным данным: обращение через поддержку личного кабинета SaulInfo. Перед публичным запуском владелец сервиса должен указать реквизиты оператора персональных данных."),
            "privacy_data_categories": site_lines("site_privacy_data_categories", [
                "учётные данные: e-mail, пароль в виде хеша, отображаемое имя, идентификаторы аккаунта",
                "данные клиентского профиля: баланс, статус доступа, активные ключи, хосты, сроки действия ключей",
                "платёжные данные: сумма, валюта, тариф, скидка, способ оплаты, ID платежа, статус, дата, результат операции и служебные ответы платёжного провайдера",
                "данные поддержки: тема тикета, сообщения, статусы, ответы администратора, добровольно загруженные изображения или файлы",
                "технические и защитные данные: дата входа, дата обновления аккаунта, сведения о временном ограничении доступа",
            ]),
            "privacy_payments_text": site_setting("site_privacy_payments_text", "Для оплаты SaulInfo может передавать платёжному провайдеру минимально необходимые данные заказа: сумму, идентификатор платежа, описание покупки, адреса возврата и callback. Данные банковских карт вводятся на стороне платёжного провайдера и не сохраняются в SaulInfo."),
            "privacy_support_media_text": site_setting("site_privacy_support_media_text", "Изображения и фотографии, загруженные пользователем в поддержку, используются только для разбора обращения. Пользователь сам решает, загружать ли такие материалы. SaulInfo не использует фотографии для биометрической идентификации."),
            "privacy_retention_text": site_setting("site_privacy_retention_text", "Аккаунты без активных ключей могут удаляться после длительной неактивности. Тикеты поддержки и вложения хранятся, пока обращение открыто или пока это необходимо для истории поддержки, после чего могут быть удалены. Платёжные сведения могут храниться дольше, если они нужны для учёта операций, возвратов, споров или требований закона."),
            "privacy_user_rights_text": site_setting("site_privacy_user_rights_text", "Пользователь может запросить уточнение, обновление, ограничение обработки или удаление данных, если это не противоречит обязанностям по хранению платёжной и иной обязательной информации."),
            "privacy_security_text": site_setting("site_privacy_security_text", "SaulInfo применяет разделение пользовательского кабинета и панели, хранит пароли в виде хеша, ограничивает административные функции ролями и удаляет вложения тикета при закрытии или удалении обращения, если это настроено в рабочем контуре."),
            "consent_title": site_setting("site_consent_title", "Согласие на обработку персональных данных"),
            "consent_intro": site_setting("site_consent_intro", "Этот текст используется при регистрации аккаунта SaulInfo и оформлен для обработки персональных данных в РФ с учётом требований 152-ФЗ."),
            "consent_confirmation_text": site_setting("site_consent_confirmation_text", "Создавая аккаунт, пользователь свободно, своей волей и в своём интересе даёт согласие на обработку персональных данных для работы личного кабинета, покупки и продления VPN-ключей, обработки платежей и обращения в поддержку."),
            "consent_data_categories": site_lines("site_consent_data_categories", [
                "e-mail, имя или отображаемое имя, идентификаторы аккаунта сайта и связанного клиентского профиля",
                "технические данные входа и безопасности: время входа, статус доступа, сведения о бане или ограничении",
                "данные заказов и платежей: выбранный тариф, сумма, валюта, скидка, способ оплаты, ID платежа, статус оплаты, дата и результат операции",
                "данные ключей доступа: выбранный хост, срок действия, статус ключа, комментарии и служебные идентификаторы подключения",
                "сообщения поддержки, вложения и добровольно прикрепляемые изображения/фотографии, если пользователь сам загрузил их в тикет",
            ]),
            "consent_purpose_text": site_setting("site_consent_purpose_text", "Данные используются для регистрации и входа, выдачи и продления ключей, учёта баланса и платежей, поддержки пользователей, предотвращения злоупотреблений, исполнения обязанностей по учёту операций и улучшения работы сервиса."),
            "consent_support_media_text": site_setting("site_consent_support_media_text", "Пользователь может добровольно прикрепить изображение или фотографию к обращению в поддержку, например скриншот ошибки. Такие материалы обрабатываются только для разбора обращения и удаляются вместе с тикетом или по правилам хранения поддержки."),
            "consent_support_media_note": site_setting("site_consent_support_media_note", "Фотографии не используются SaulInfo для установления личности, биометрической идентификации или проверки личности пользователя."),
            "consent_retention_text": site_setting("site_consent_retention_text", "Согласие действует до удаления аккаунта или до отзыва согласия пользователем. После отзыва часть данных может сохраняться только в случаях, когда это необходимо для исполнения закона, учёта платежей, защиты прав или завершения уже начатых операций."),
        }
